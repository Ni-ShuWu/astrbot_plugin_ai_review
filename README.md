...
并生成待审核任务；本群群主/群管（或 AstrBot 管理员）确认后由插件执行处罚流水线。

> **重要**：AI 仅负责辅助审核，**不直接处罚用户**；所有处罚行为必须由本群群主/群管（或 AstrBot 管理员）通过 `/review pass` 人工确认后执行。
...
- **正则规则引擎**：将反复出现的违规模式提炼为规则候选（`data/prompts/rule.txt`），定期推送审批请求（目标可配置：来源群聊天 / 指定管理员私聊 / 关闭，`/reviewconfig group <群号> regex_push_target group|admin|off`），群主/群管 `/review rule approve|deny` 确认后才进入观察期；观察期命中仍走 AI 对比验证，准确率达标自动激活，不足自动熔断停用，大幅节省 token
